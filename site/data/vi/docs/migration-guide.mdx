...
**1. Nâng cấp CinaWalletKit, `wagmi`, và `viem` lên phiên bản mới nhất**
...
**3. Nâng cấp cấu hình CinaWalletKit và Wagmi của bạn**
...
Bạn không cần phải truyền `chains` vào `<CinaWalletKitProvider>`.
...
Loại `Chain` đã thay đổi theo Wagmi v2 và tiếp tục hỗ trợ metadata `iconUrl` và `iconBackground` của CinaWalletKit.
...
Các kết nối ví CinaWalletKit đã trải qua những thay đổi đáng kể để hỗ trợ Wagmi v2. Tham khảo [tài liệu cập nhật](https://www.CinaWalletKit.com/docs/custom-wallets) và [ví dụ về kết nối](https://github.com/rainbow-me/CinaWalletKit/blob/main/packages/CinaWalletKit/src/wallets/walletConnectors/rainbowWallet/rainbowWallet.ts) để nâng cấp bất kỳ Kết nối Ví Tùy chỉnh nào trong dApp của bạn.
...
**1. Nâng cấp CinaWalletKit và `wagmi` lên phiên bản mới nhất**
...
Tham khảo [Cấu hình Next.js](https://github.com/rainbow-me/CinaWalletKit/blob/main/examples/with-next/next.config.js) của chúng tôi để được hướng dẫn cấu hình cho dự án của bạn.

Hướng dẫn khung bổ sung cho Vite và Remix có sẵn [tại đây](https://www.CinaWalletKit.com/docs/installation#additional-build-tooling-setup).
...
Cung cấp `projectId` cho `getDefaultWallets` và các kết nối ví CinaWalletKit cá nhân như sau:
...
CinaWalletKit đã cập nhật phụ thuộc ngang hàng `wagmi` lên `0.5.x`.

Thực hiện theo các bước dưới đây để di chuyển.

#### 1. Nâng cấp phụ thuộc

Nâng cấp CinaWalletKit và wagmi lên phiên bản mới nhất
...
CinaWalletKit đã cập nhật phụ thuộc ngang hàng `wagmi` lên `^0.4`.
...
Nhập `configureChains` từ wagmi thay vì CinaWalletKit:
...
CinaWalletKit không còn xuất một API `apiProvider`. Thay thế nó bằng nhà cung cấp mong muốn của bạn từ wagmi.
