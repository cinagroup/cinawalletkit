...
**Обновите CinaWalletKit, `wagmi`, и `viem` до последних версий**
...
**Обновите конфигурации CinaWalletKit и Wagmi**
...
Больше не нужно передавать `chains` в `<CinaWalletKitProvider>`.
...
Тип `Chain` изменился в соответствии с Wagmi v2 и продолжает поддерживать метаданные CinaWalletKit `iconUrl` и `iconBackground`.
...
Коннекторы кошелька CinaWalletKit подверглись значительным изменениям для поддержки Wagmi v2. Ссылка на [обновленную документацию](https://www.CinaWalletKit.com/docs/custom-wallets) и [пример коннектора](https://github.com/rainbow-me/CinaWalletKit/blob/main/packages/CinaWalletKit/src/wallets/walletConnectors/rainbowWallet/rainbowWallet.ts) для обновления любых Пользовательских Коннекторов Кошельков в вашем dApp.
...
**Обновите CinaWalletKit и `wagmi` до их последней версии**
...
См. наш пример [Конфигурация Next.js](https://github.com/rainbow-me/CinaWalletKit/blob/main/examples/with-next/next.config.js) для рекомендаций по настройке вашего проекта.

Дополнительные руководства по фреймворку для Vite и Remix доступны [здесь](https://www.CinaWalletKit.com/docs/installation#additional-build-tooling-setup).
...
CinaWalletKit принял коннектор `WalletConnectLegacyConnector` в `wagmi` для поддержки WalletConnect v1. Поддержка WalletConnect v2 и `WalletConnectConnector` будет доступна в качестве патча без критических изменений.

Кошельки будут автоматически переведены на последующие релизы.

Каждое dApp должно теперь предоставить `projectId` от [WalletConnect Cloud](https://cloud.walletconnect.com/) для активации WalletConnect v2. Это должно быть завершено до выключения серверов моста WalletConnect v1 28 июня 2023 года. CinaWalletKit будет тихо предпочтительнее v1 для всех кошельков, если `projectId` не указан.

Следуйте инструкциям ниже для выполнения миграции.

#### Обновите CinaWalletKit и `wagmi` до последней версии
...
Предоставьте `projectId` для `getDefaultWallets` и индивидуальных соединителей кошелька CinaWalletKit следующим образом:
...
CinaWalletKit обновил зависимость уровня `wagmi` до `0.5.x`.
...
CinaWalletKit обновил пир-зависимость `wagmi` до `^0.4`.
...
Импорт `configureChains` из wagmi вместо CinaWalletKit:
...
CinaWalletKit больше не экспортирует API `apiProvider`. Замените его на предпочитаемого провайдера из wagmi.
