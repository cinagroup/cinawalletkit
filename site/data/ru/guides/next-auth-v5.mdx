---
title: Обновление CinaWalletKit SIWE до NextAuth v5
description: Перенос адаптера SIWE для CinaWalletKit с NextAuth v4 на v5
---

# Перенос CinaWalletKit SIWE на NextAuth v5

Пакет `@cinagroup/CinaWalletKit-siwe-next-auth` теперь поддерживает NextAuth v5. Это значительное обновление для приложений, использующих адаптер SIWE NextAuth, поскольку NextAuth изменил свою серверную конфигурацию API, API помощника сессий и внутренний процесс CSRF.
...
Следуйте приведенным ниже инструкциям, если ваше приложение в настоящее время использует `@cinagroup/CinaWalletKit-siwe-next-auth` с NextAuth v4.

**1. Обновите зависимости**

Установите поддерживаемую CinaWalletKit версию пакета NextAuth v5.
...
В реализации v4 примеры CinaWalletKit сравнивали nonce SIWE с `getCsrfToken` внутри `authorize`.
...
Внутренние cookies NextAuth v5 используют префикс Auth.js `authjs`, включая `authjs.csrf-token` или `__Host-authjs.csrf-token` в зависимости от настроек secure-cookie. Вам не следует самим разбирать эти cookies в SIWE-потоке CinaWalletKit.
...
Полную настройку NextAuth v5 вы можете увидеть в [примере `with-next-siwe-next-auth`](https://github.com/rainbow-me/CinaWalletKit/tree/main/examples/with-next-siwe-next-auth).

Если вы также обновляетесь с более старой интеграции CinaWalletKit SIWE, ознакомьтесь с журналом изменений `0.5.0` для миграции `viem/siwe` и журналом изменений `0.3.0` для более раннего изменения формы запроса `getCsrfToken`.
