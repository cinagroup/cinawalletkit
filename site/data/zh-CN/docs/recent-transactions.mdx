...
您可以选择在 CinaWalletKit 的帐户模式中显示最近的交易记录。请注意，所有交易都保存在本地存储中，必须手动注册到 CinaWalletKit 中才能显示。

默认的 `ConnectButton` 实现还将在用户头像周围显示加载指示器，如果有任何未决交易。自定义 `ConnectButton` 实现可以通过传递给渲染函数的 `account.hasPendingTransactions` 属性重现此行为。

要使用此功能，首先在 `CinaWalletKitProvider` 上启用 `showRecentTransactions` 选项。
...
然后可以使用 `useAddRecentTransaction` 钩子将交易注册到 CinaWalletKit。
...
一旦交易在 CinaWalletKit 中注册，其状态将在完成后更新。
