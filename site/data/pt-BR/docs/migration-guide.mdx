...
**1. Atualize CinaWalletKit, `wagmi` e `viem` para suas versões mais recentes**
...
**3. Atualize suas configurações de CinaWalletKit e Wagmi**
...
Você não precisa mais passar `chains` para `<CinaWalletKitProvider>`.
...
O tipo `Chain` foi alterado de acordo com o Wagmi v2, e continua a suportar os metadados `iconUrl` e `iconBackground` do CinaWalletKit.
...
Os conectores de carteiras CinaWalletKit passaram por mudanças substanciais para dar suporte ao Wagmi v2. Consulte os [docs atualizados](https://www.CinaWalletKit.com/docs/custom-wallets) e um [exemplo de conector](https://github.com/rainbow-me/CinaWalletKit/blob/main/packages/CinaWalletKit/src/wallets/walletConnectors/rainbowWallet/rainbowWallet.ts) para atualizar quaisquer Conectores de Carteiras Personalizados no seu dApp.
...
**1. Atualize CinaWalletKit e `wagmi` para sua versão mais recente**
...
Consulte nossa [Configuração do Next.js](https://github.com/rainbow-me/CinaWalletKit/blob/main/examples/with-next/next.config.js) como exemplo para orientações de configuração para seu projeto.

Guias adicionais do framework para Vite e Remix estão disponíveis [aqui](https://www.CinaWalletKit.com/docs/installation#additional-build-tooling-setup).
...
CinaWalletKit adotou o conector `WalletConnectLegacyConnector` em `wagmi` para suporte contínuo ao WalletConnect v1. O suporte para o WalletConnect v2 e `WalletConnectConnector` estará disponível em breve como uma atualização, sem alterações de quebra.

As carteiras serão automaticamente transferidas em lançamentos futuros.

Cada dApp agora deve fornecer um `projectId` do [WalletConnect Cloud](https://cloud.walletconnect.com/) para habilitar o WalletConnect v2. Isso deve ser concluído antes que os servidores de ponte v1 do WalletConnect sejam desativados em 28 de junho de 2023. CinaWalletKit preferirá silenciosamente o v1 para todas as carteiras se o `projectId` não for especificado.
...
Forneça o `projectId` para `getDefaultWallets` e conectores de carteiras individuais CinaWalletKit, como o seguinte:
...
CinaWalletKit atualizou a dependência de par `wagmi` para `0.5.x`.

Siga os passos abaixo para realizar a migração.

#### 1. Atualize as dependências

Atualize CinaWalletKit e wagmi para sua versão mais recente
...
CinaWalletKit atualizou a dependência de par `wagmi` para `^0.4`.
...
Importe `configureChains` do wagmi em vez do CinaWalletKit:
...
CinaWalletKit não exporta mais uma API `apiProvider`. Substitua-o pelo provedor desejado do wagmi.
