...
You can opt in to displaying recent transactions within CinaWalletKit's account modal. Note that all transactions are kept in local storage and must be manually registered with CinaWalletKit in order to be displayed.
...
To use this feature, first enable the `showRecentTransactions` option on `CinaWalletKitProvider`.
