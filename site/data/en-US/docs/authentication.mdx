...
While it's possible to [integrate with custom back-ends and message formats,](/docs/custom-authentication) CinaWalletKit provides first-class support for [Sign-In with Ethereum](https://login.xyz) and [NextAuth](https://next-auth.js.org).

### Set up Sign-In with Ethereum and NextAuth

#### Install

Install the `@cinagroup/CinaWalletKit-siwe-next-auth` package and NextAuth v5.
...
See the complete [`with-next-siwe-next-auth` example](https://github.com/rainbow-me/CinaWalletKit/tree/main/examples/with-next-siwe-next-auth) for SIWE validation, nonce checks, and server-side verification.

#### Set up the provider

In your `App` component, import `CinaWalletKitSiweNextAuthProvider`.

```tsx
import { CinaWalletKitSiweNextAuthProvider } from '@cinagroup/CinaWalletKit-siwe-next-auth';
```

Wrap `CinaWalletKitProvider` with `CinaWalletKitSiweNextAuthProvider`, ensuring it's nested within NextAuth's `SessionProvider` so that it has access to the session.
...
With `CinaWalletKitSiweNextAuthProvider` in place, your users will now be prompted to authenticate by signing a message once they've connected their wallet.

#### Customize the SIWE message options

You can customize the [SIWE message options](https://viem.sh/docs/siwe/utilities/createSiweMessage#parameters) by passing a function to the `getSiweMessageOptions` prop on `CinaWalletKitSiweNextAuthProvider`.
