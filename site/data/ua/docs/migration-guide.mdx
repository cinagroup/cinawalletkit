...
**1. Оновіть CinaWalletKit, `wagmi`, і `viem` до останніх версій**
...
**3. Оновіть конфігурації CinaWalletKit і Wagmi**
...
Вам більше не потрібно передавати `chains` для `<CinaWalletKitProvider>`.
...
Тип `Chain` змінено відповідно до Wagmi v2 і продовжує підтримувати метадані CinaWalletKit `iconUrl` та `iconBackground`.
...
Конектори гаманців CinaWalletKit зазнали значних змін для підтримки Wagmi v2. Зверніться до [оновленої документації](https://www.CinaWalletKit.com/docs/custom-wallets) та [прикладу конектора](https://github.com/rainbow-me/CinaWalletKit/blob/main/packages/CinaWalletKit/src/wallets/walletConnectors/rainbowWallet/rainbowWallet.ts) для оновлення будь-яких кастомних конекторів гаманців у вашому dApp.
...
**Оновіть CinaWalletKit та wagmi до останньої версії**
...
Дивіться наш приклад [Конфігурації Next.js](https://github.com/rainbow-me/CinaWalletKit/blob/main/examples/with-next/next.config.js) для керівництва з налаштування вашого проекту.

Додаткові посібники для Vite та Remix доступні [тут](https://www.CinaWalletKit.com/docs/installation#additional-build-tooling-setup).
...
CinaWalletKit прийняв конектор `WalletConnectLegacyConnector` у `wagmi` для продовження підтримки WalletConnect v1. Підтримка WalletConnect v2 та `WalletConnectConnector` буде незабаром доступна як патч, без змін, що розривають сумісність.

Гаманці будуть автоматично переведені у майбутніх релізах.

Кожний dApp тепер повинен надати [WalletConnect Cloud](https://cloud.walletconnect.com/) `projectId` для активації WalletConnect v2. Це має бути завершено до відключення серверів моста WalletConnect v1 28 червня 2023 року. CinaWalletKit тихо віддасть перевагу v1 для всіх гаманців, якщо `projectId` не вказано.
...
Подайте `projectId` до `getDefaultWallets` та індивідуальних конекторів гаманців CinaWalletKit наступним чином:
...
### CinaWalletKit оновив залежність `wagmi` до версії `0.5.x`.
...
CinaWalletKit оновив залежність `wagmi` до версії `^0.4`.
...
#### Імпортуйте `configureChains` з wagmi замість CinaWalletKit:
...
CinaWalletKit більше не експортує API `apiProvider`. Замініть його бажаним провайдером з wagmi.
