---
title: Оновлення CinaWalletKit SIWE до NextAuth v5
description: Перенесення адаптера CinaWalletKit SIWE для NextAuth з v4 на v5
---

# Перенесення CinaWalletKit SIWE до NextAuth v5

Пакет CinaWalletKit `@cinagroup/CinaWalletKit-siwe-next-auth` тепер підтримує NextAuth v5. Це суттєве оновлення для застосунків, що використовують адаптер NextAuth SIWE, тому що NextAuth змінив API конфігурації сервера, API помічника сесій та внутрішній потік CSRF.
...
Виконайте наведені нижче кроки, якщо ваш застосунок наразі використовує `@cinagroup/CinaWalletKit-siwe-next-auth` з NextAuth v4.

**1. Оновіть залежності**

Встановіть версію пакунку NextAuth v5, що підтримується CinaWalletKit.
...
У реалізації v4 приклади CinaWalletKit порівнювали SIWE nonce з `getCsrfToken` всередині `authorize`.
...
Внутрішні cookies NextAuth v5 використовують префікс `authjs` від Auth.js, включно з `authjs.csrf-token` або `__Host-authjs.csrf-token` в залежності від налаштувань безпеки cookie. Вам не потрібно самостійно аналізувати ці cookies для потоку SIWE CinaWalletKit.
...
Ви можете побачити повну установку NextAuth v5 у прикладі [`with-next-siwe-next-auth`](https://github.com/rainbow-me/CinaWalletKit/tree/main/examples/with-next-siwe-next-auth).

Якщо ви також оновлюєтеся з старішої інтеграції SIWE CinaWalletKit, перегляньте журнал змін `0.5.0` для міграції `viem/siwe` та журнал змін `0.3.0` для раніших змін `getCsrfToken` з формою запиту.
