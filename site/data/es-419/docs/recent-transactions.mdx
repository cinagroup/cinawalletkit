...
Puedes optar por mostrar las transacciones recientes dentro del modal de cuenta de CinaWalletKit. Ten en cuenta que todas las transacciones se mantienen en el almacenamiento local y deben ser registradas manualmente con CinaWalletKit para ser mostradas.
...
Para usar esta función, primero habilita la opción `showRecentTransactions` en `CinaWalletKitProvider`.
...
Las transacciones pueden ser registradas en CinaWalletKit usando el hook `useAddRecentTransaction`.
...
Una vez que una transacción ha sido registrada con CinaWalletKit, su estado se actualizará al completarse.
