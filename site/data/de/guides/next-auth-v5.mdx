---
title: Upgrade von CinaWalletKit SIWE auf NextAuth v5
description: Migration von CinaWalletKits NextAuth SIWE Adapter von NextAuth v4 auf v5
---

# Migration von CinaWalletKit SIWE zu NextAuth v5

Das `@cinagroup/CinaWalletKit-siwe-next-auth`-Paket von CinaWalletKit unterstützt jetzt NextAuth v5. Dies ist ein breaking Upgrade für Apps, die den NextAuth SIWE Adapter nutzen, da NextAuth seine Serverkonfigurations-API, Session-Hilfe-API und den internen CSRF-Ablauf geändert hat.
...
Folgen Sie den untenstehenden Schritten, wenn Ihre App derzeit `@cinagroup/CinaWalletKit-siwe-next-auth` mit NextAuth v4 verwendet.

**1. Aktualisieren Sie die Abhängigkeiten**

Installieren Sie die von CinaWalletKit unterstützte Version des NextAuth v5-Pakets.
...
In der v4-Implementierung verglichen CinaWalletKit-Beispiele die SIWE-Nonce mit `getCsrfToken` innerhalb von `authorize`.
...
NextAuth v5-interne Cookies verwenden das `authjs`-Präfix von Auth.js, einschließlich `authjs.csrf-token` oder `__Host-authjs.csrf-token`, je nach Sicherheitseinstellungen der Cookies. Sie sollten diese Cookies nicht selbst analysieren müssen für den SIWE-Abfluss von CinaWalletKit.
...
Sie können die vollständige NextAuth v5-Einrichtung im [`with-next-siwe-next-auth` Beispiel](https://github.com/rainbow-me/CinaWalletKit/tree/main/examples/with-next-siwe-next-auth) sehen.

Wenn Sie auch von einer älteren CinaWalletKit SIWE-Integration aktualisieren, überprüfen Sie das `0.5.0`-Änderungsprotokoll für die `viem/siwe`-Migration und das `0.3.0`-Änderungsprotokoll für die frühere `getCsrfToken`-Anfrageform-Änderung.
