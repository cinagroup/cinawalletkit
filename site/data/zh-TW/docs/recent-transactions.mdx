...
您可以選擇在 CinaWalletKit 的帳戶模式中顯示最近的交易。請注意，所有交易都保存在本地存儲中，並且必須手動向 CinaWalletKit 註冊才能顯示。
...
要使用此功能，首先在 `CinaWalletKitProvider` 上啟用 `showRecentTransactions` 選項。
...
然後可以使用 `useAddRecentTransaction` 鉤子與 CinaWalletKit 註冊交易。
...
一旦交易已經在 CinaWalletKit 註冊，其狀態將在完成時更新。
