...
CinaWalletKit 錢包連接器經過大幅更改以支援 Wagmi v2。請參考[更新的文檔](https://www.CinaWalletKit.com/docs/custom-wallets)和[示例連接器](https://github.com/rainbow-me/CinaWalletKit/blob/main/packages/CinaWalletKit/src/wallets/walletConnectors/rainbowWallet/rainbowWallet.ts)以升級您 dApp 中的自定義錢包連接器。
...
**1. 升級 CinaWalletKit 和 `wagmi` 到最新版本**
...
請參考我們的 [Next.js 配置範例](https://github.com/rainbow-me/CinaWalletKit/blob/main/examples/with-next/next.config.js)以獲取您的項目配置指導。

可在此處獲得 Vite 和 Remix 的附加框架指南：[其他建構工具設置](https://www.CinaWalletKit.com/docs/installation#additional-build-tooling-setup)
...
CinaWalletKit 已採納 `WalletConnectLegacyConnector` 連接器以持續支援 WalletConnect v1。對 WalletConnect v2 和 `WalletConnectConnector` 的支援很快將作為修補釋出而無重大變更。

將在未來的版本中自動過渡錢包。

現在，每個 dApp 必須提供 [WalletConnect Cloud](https://cloud.walletconnect.com/) 的 `projectId` 以開啟 WalletConnect v2 功能。需在 2023 年 6 月 28 日 WalletConnect v1 桥服務器關閉之前完成。現若未指定 `projectId`，CinaWalletKit 將靜默偏好所有錢包的 v1。
...
將 `projectId` 提供給 `getDefaultWallets` 和類似的 individuell CinaWalletKit 錢包連接器：
...
CinaWalletKit 已更新 `wagmi` 的等級依賴項至 `0.5.x`。

請遵循以下步驟進行遷移。

#### 1. 升級依賴

升級 CinaWalletKit 和 wagmi 到最新版本
...
CinaWalletKit 已更新 `wagmi` 的等級依賴項至 `^0.4`。
...
從 wagmi 而非 CinaWalletKit 匯入 `configureChains`：
...
CinaWalletKit 不再匯出 `apiProvider` API。用您想要的 wagmi 提供者替換它。
