...
CinaWalletKit錢包連接器經過大量更改以支持Wagmi v2。參考[更新文檔](https://www.CinaWalletKit.com/docs/custom-wallets)和[示例連接器](https://github.com/rainbow-me/CinaWalletKit/blob/main/packages/CinaWalletKit/src/wallets/walletConnectors/rainbowWallet/rainbowWallet.ts)來升級您dApp中的任何自訂錢包連接器。
...
**1. 升級CinaWalletKit和`wagmi`至最新版本**
...
參考我們的 [Next.js 配置](https://github.com/rainbow-me/CinaWalletKit/blob/main/examples/with-next/next.config.js)範例，為您的項目提供配置指引。

可用的Vite和Remix的額外框架指南[此處](https://www.CinaWalletKit.com/docs/installation#additional-build-tooling-setup)。
...
CinaWalletKit已採用`walletConnectLegacyConnector`連接器在`wagmi`中以持續支持WalletConnect v1。對WalletConnect v2和`walletConnectConnector`的支持將很快作為補丁版提供，無重大變更。

未來的版本中錢包將自動過渡。

每個dApp現在必須提供一個[WalletConnect Cloud](https://cloud.walletconnect.com/) `projectId`來啟用WalletConnect v2。在2023年6月28日WalletConnect v1網橋服務器關閉之前，必須完成此操作。如果未指定`projectId`，CinaWalletKit將默認使用v1。
...
將`projectId`提供給`getDefaultWallets`和單個CinaWalletKit錢包連接器，例如：
...
CinaWalletKit已更新`wagmi`對等依賴至`0.5.x`。

請按照以下步驟進行遷移。

#### 1. 升級依賴

升級CinaWalletKit和wagmi至其最新版本
...
CinaWalletKit已更新`wagmi`對等依賴至`^0.4`。
...
從wagmi導入`configureChains`而不是CinaWalletKit：
...
CinaWalletKit不再導出`apiProvider` API。用您想要的wagmi提供者替代它。
