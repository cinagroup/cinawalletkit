...
您可以選擇在 CinaWalletKit 的帳戶模態窗口中顯示最近的交易記錄。請注意，所有交易記錄都保存在本地存儲中，必須手動註冊到 CinaWalletKit 中才能顯示。
...
要使用此功能，首先需要在 `CinaWalletKitProvider` 上啟用 `showRecentTransactions` 選項。
...
然後可以使用 `useAddRecentTransaction` 鉤子在 CinaWalletKit 中註冊交易。
...
一旦交易註冊到 CinaWalletKit，其狀態將在完成時更新。
