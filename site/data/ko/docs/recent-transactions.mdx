...
CinaWalletKit의 계정 모달 내에서 최근 트랜잭션 표시를 선택적으로 설정할 수 있습니다. 모든 트랜잭션은 로컬 저장소에 보관되며 표시되기 위해서는 CinaWalletKit에 수동으로 등록되어야 합니다.
...
이 기능을 사용하려면 먼저 `CinaWalletKitProvider`에서 `showRecentTransactions` 옵션을 활성화하십시오.
...
그런 다음 `useAddRecentTransaction` 훅을 사용하여 CinaWalletKit에 트랜잭션을 등록할 수 있습니다.
...
트랜잭션이 CinaWalletKit에 등록되면 완료 시 상태가 업데이트됩니다.
