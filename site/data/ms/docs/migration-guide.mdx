...
**1. Naik taraf CinaWalletKit, `wagmi`, dan `viem` ke versi terkini**
...
**3. Naik taraf konfigurasi CinaWalletKit dan Wagmi anda**
...
Anda tidak lagi perlu menyampaikan `chains` kepada `<CinaWalletKitProvider>`.
...
Jenis `Chain` telah berubah selaras dengan Wagmi v2, dan terus menyokong metadata `iconUrl` dan `iconBackground` CinaWalletKit.
...
Penyambung dompet CinaWalletKit telah mengalami perubahan yang ketara untuk menyokong Wagmi v2. Rujuk [dokumen yang dikemas kini](https://www.CinaWalletKit.com/docs/custom-wallets) dan [penyambung contoh](https://github.com/rainbow-me/CinaWalletKit/blob/main/packages/CinaWalletKit/src/wallets/walletConnectors/rainbowWallet/rainbowWallet.ts) untuk menaik taraf sebarang Penyambung Dompet Khusus dalam dApp anda.
...
**1. Naik taraf CinaWalletKit dan `wagmi` ke versi terkini**
...
Rujuk sampel [Next.js Config](https://github.com/rainbow-me/CinaWalletKit/blob/main/examples/with-next/next.config.js) kami untuk panduan konfigurasi bagi projek anda.

Panduan kerangka tambahan untuk Vite dan Remix tersedia [di sini](https://www.CinaWalletKit.com/docs/installation#additional-build-tooling-setup).
...
CinaWalletKit telah mengadopsi penyambung `WalletConnectLegacyConnector` dalam `wagmi` untuk meneruskan sokongan WalletConnect v1. Sokongan untuk WalletConnect v2 dan `WalletConnectConnector` akan tersedia sebagai pelepasan tampalan tanpa perubahan besar.

Dompet akan ditekan secara automatik dalam pelepasan masa depan.

Setiap dApp kini mesti menyediakan `projectId` [WalletConnect Cloud](https://cloud.walletconnect.com/) untuk membolehkan WalletConnect v2. Ini mesti disiapkan sebelum pelayan jambatan WalletConnect v1 ditutup pada 28 Jun, 2023. CinaWalletKit akan lebih suka v1 untuk semua dompet jika `projectId` tidak disebutkan.
...
Sediakan `projectId` kepada `getDefaultWallets` dan penyambung dompet CinaWalletKit individu seperti berikut:
...
CinaWalletKit telah mengemaskini kebergantungan rakan sebaya `wagmi` ke `0.5.x`.

Ikut langkah di bawah untuk beralih.

#### 1. Naik taraf kebergantungan

Naik taraf CinaWalletKit dan wagmi ke versi terkini
...
CinaWalletKit telah mengemaskini kebergantungan rakan sebaya `wagmi` ke `^0.4`.
...
Import `configureChains` dari wagmi dan bukan CinaWalletKit:
...
CinaWalletKit tidak lagi mengeksport API `apiProvider`. Gantikannya dengan penyedia yang diingini dari wagmi.
