...
CinaWalletKitのアカウントモーダル内で最近の取引を表示することを選択できます。すべての取引はローカルストレージに保存され、表示するには手動でCinaWalletKitに登録する必要があります。
...
この機能を使用するには、まず`CinaWalletKitProvider`で`showRecentTransactions`オプションを有効にしてください。
...
取引は`useAddRecentTransaction`フックを使用してCinaWalletKitに登録できます。
...
取引がCinaWalletKitに登録されると、そのステータスは完了時に更新されます。
