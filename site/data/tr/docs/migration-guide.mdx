...
**CinaWalletKit, `wagmi` ve `viem`'ı en son sürümlerine yükseltin**
...
**CinaWalletKit ve Wagmi yapılandırmalarınızı güncelleyin**
...
**CinaWalletKit Sağlayıcı**

`<CinaWalletKitProvider>`'a artık `chains` geçirmeniz gerekmiyor.
...
Wagmi v2 ile uyumlu olarak `Chain` tipi değişmiştir ve CinaWalletKit'in `iconUrl` ve `iconBackground` meta verilerini desteklemeye devam etmektedir.
...
DApp'inizdeki Özel Cüzdan Bağlayıcılarını güncellemek için [güncellenmiş belgelere](https://www.CinaWalletKit.com/docs/custom-wallets) ve bir [örnek bağlayıcıya](https://github.com/rainbow-me/CinaWalletKit/blob/main/packages/CinaWalletKit/src/wallets/walletConnectors/rainbowWallet/rainbowWallet.ts) başvurun.
...
**CinaWalletKit ve `wagmi` yi en son sürümüne yükseltin**
...
Projeleriniz için yapılandırma kılavuzu olarak bizim [Next.js Config](https://github.com/rainbow-me/CinaWalletKit/blob/main/examples/with-next/next.config.js) örneğine bakın.

Vite ve Remix için ek CinaWalletKit framework rehberleri mevcuttur [here](https://www.CinaWalletKit.com/docs/installation#additional-build-tooling-setup).
...
CinaWalletKit `wagmi`'de WalletConnect v1 desteği için `WalletConnectLegacyConnector` bağlayıcısını benimsemiştir. WalletConnect v2 ve `WalletConnectConnector` için destek, kırılma değişiklikleri olmadan bir yama sürümü olarak yakında sunulacak.

Cüzdanlar gelecek sürümlerde otomatik olarak geçiş yapacak.

WalletConnect v2'yi etkinleştirmek için her dApp'in artık bir [WalletConnect Cloud](https://cloud.walletconnect.com/) `projectId` sağlaması gerekmektedir. 28 Haziran 2023 tarihinde WalletConnect v1 köprü sunucuları kapatılmadan önce bu tamamlanmalıdır. `projectId` belirtilmemişse CinaWalletKit, tüm cüzdanlar için sessizce v1'i tercih edecektir.

Geçiş yapmak için aşağıdaki adımları izleyin.

#### CinaWalletKit ve `wagmi` yi en güncel versiyonlarına yükseltin
...
`getDefaultWallets` ve bireysel CinaWalletKit cüzdan bağlantılarına `projectId` sağlayın aşağıdaki gibi:
...
CinaWalletKit, `wagmi` eşdeğer bağımlılığını `0.5.x`'e güncelledi.

Geçiş yapmak için aşağıdaki adımları izleyin.

#### Bağımlılıkları Güncelle

CinaWalletKit ve wagmi'yi en son sürüme yükseltin
...
CinaWalletKit, `wagmi` eşbağımlılığını `^0.4`a güncelledi.
...
`configureChains`ı CinaWalletKit yerine wagmi'den içe aktar:
...
CinaWalletKit artık bir `apiProvider` API'sı ihraç etmiyor. Yerine wagmi'den istediğiniz sağlayıcıyı kullanın.
