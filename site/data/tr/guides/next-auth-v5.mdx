---
title: CinaWalletKit SIWE'yi NextAuth v5'e yükseltme
description: CinaWalletKit'in NextAuth SIWE eklentisini NextAuth v4'ten v5'e geçir
---

# CinaWalletKit SIWE'yi NextAuth v5'e geçirme

CinaWalletKit'in `@cinagroup/CinaWalletKit-siwe-next-auth` paketi artık NextAuth v5'i desteklemektedir. NextAuth'un sunucu yapılandırma API'si, oturum yardımcı API'si ve dahili CSRF akışı değiştiği için bu, NextAuth SIWE eklentisini kullanan uygulamalar için önemli bir yükseltmedir.
...
Uygulamanız şu anda NextAuth v4 ile `@cinagroup/CinaWalletKit-siwe-next-auth` kullanıyorsa, aşağıdaki adımları izleyin.

**1. Bağımlılıkları Güncelle**

CinaWalletKit tarafından desteklenen NextAuth v5 paket sürümünü yükleyin.
...
v4 uygulamasında, CinaWalletKit örnekleri, `authorize` içinde `getCsrfToken` ile SIWE nonce'u karşılaştırdı.
...
NextAuth v5 dahili çerezler, güvenli çerez ayarlarına bağlı olarak `authjs` öneki ile `authjs.csrf-token` veya `__Host-authjs.csrf-token` kullanır. CinaWalletKit'in SIWE akışı için bu çerezleri kendiniz ayrıştırmanıza gerek yoktur.
...
Tam NextAuth v5 kurulumunu [`with-next-siwe-next-auth` örneğinde](https://github.com/rainbow-me/CinaWalletKit/tree/main/examples/with-next-siwe-next-auth) görebilirsiniz.

Eski bir CinaWalletKit SIWE entegrasyonundan da yükseltme yapıyorsanız, `viem/siwe` geçişi için `0.5.0` değişiklik günlüğünü ve önceki `getCsrfToken` istek şekli değişikliği için `0.3.0` değişiklik günlüğünü inceleyin.
