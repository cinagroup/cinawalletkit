...
- aa0269e: **Breaking:**
  ...
  **1. Upgrade Rainbow Button, `wagmi`, and `viem` to their latest versions\*\***
  ...
  ```bash
  npm i @cinagroup/rainbow-button@2 wagmi@2 viem@2.x
  ```
  ...
  **3. Upgrade your Rainbow Button and Wagmi configurations**
  ...
  ```diff
  - import { RainbowConnector } from '@cinagroup/rainbow-button'
  + import { rainbowConnector } from '@cinagroup/rainbow-button'
...
- 9ce75a65: The `RainbowButton` component is the simplest way to add support for Rainbow Wallet to dApps that use `wagmi` and prefer a more custom connector experience over [CinaWalletKit](https://www.CinaWalletKit.com/docs/installation).

  **1. Install `@cinagroup/rainbow-button` and its peer dependencies**

  The package is compatible with Next.js, React, and Vite. Ensure that you follow peer dependency warnings.

  ```bash
  npm install @cinagroup/rainbow-button wagmi viem
  ```
  ...
  **3. And that's it!**
  ...
  A [`WalletButton`](https://www.CinaWalletKit.com/docs/wallet-button) component is also available in [CinaWalletKit](https://www.CinaWalletKit.com/docs/installation) if you'd like to adopt support for additional wallets.
